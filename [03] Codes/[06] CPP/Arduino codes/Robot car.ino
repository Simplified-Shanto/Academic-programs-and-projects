...
//#define varY                  A1                            //X axis of Joystick
//#define varX                  A0                           //Y axis of Joystick
#define xTrigger              D3    //This pin will be turned HIGH and the other LOW to read X axis potentiometer's value via A0
#define yTrigger              D4    //This pin will be turned HIGH and the other LOW to read Y axis potentiometer's value via A0
...
#define stickButton           D0                            //Built in button of joystick
...
unsigned short settingsValue[noSettings] = {70,        170,             80,          120,              490,              530,             90,         300,        200,          150,           6,                  0       }; //Initial parameters
...
void loop() {
  ...
  if(Mode==2)       //In obstacle avoidance mode. 
  {
    ...
    if(Mode==1)               //Settings mode
    {
      ...
      {
         if(  xAxisValue()
         < 200)  //whether the joystick is moved right. 
          {
         ...
         > 700)   //whether the joystick is moved left. 
          {
           settingsValue[settingsIndex]-=(settingsIndex==noSettings-2?1:scaleFactor);    //If we are changing safe distance limit, change by 1, or change by scale factor. 
           if( (settingsValue[settingsIndex]+1)%10==0 || (settingsValue[settingsIndex]-1)%10==0 ) { lcd.clear(); } //Clearing the probable garbage value left by last small or greater number at 
  ...
  else if(yAxisValue()< settingsValue[4] )
  {
   Serial.print('f');
    Serial.print(':'); 
 ...
 > settingsValue[5])
  {
    Serial.print('l'); 
    Serial.print(':'); 
    Serial.print(map(xAxisValue()
  , settingsValue[5], 1023, settingsValue[2], settingsValue[3])); //sending the speed. 
    Serial.print('.'); 
  }
