...
void dfs(int currentVertex) // Depth first search will start from this vertex and sequentially visit all the vertices of the graph. 
{
    printf("%d ", currentVertex );
    visited[currentVertex] = 1;
    for (int i = 0; i < V; i++)
    {
        if(graph[currentVertex][i]==1 && visited[i]!=1)
        {
            dfs(i); 
        }
    }
